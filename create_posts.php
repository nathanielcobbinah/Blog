...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["thumbnail_image"]["name"])) {
        ...
        // if ($thumbnail_imageFileType != "jpg" && $thumbnail_imageFileType != "png" && $thumbnail_imageFileType != "jpeg" && $thumbnail_imageFileType != "gif") {
        //     $thumbnail_image_err = "Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
        // }
    ...
    if (empty($title_err) && empty($content_err) && empty($category_err) && empty($thumbnail_image_err)) {
        ...
        $sql = "INSERT INTO posts (title, content, excerpt, thumbnail_image, category_id) VALUES (?, ?, ?, ?, ?)";

        if ($stmt = $conn->prepare($sql)) {
            // Bind variables to the prepared statement as parameters
            $stmt->bind_param("ssssi", $title, $content, $excerpt, $thumbnail_image, $category_id);
    ...
    <div class="max-w-3xl mx-auto bg-white p-6 rounded shadow">
        ...
        <form action="create_posts.php" method="post" enctype="multipart/form-data">
            <div class="mb-4">
                ...
                <input type="text" id="title" name="title" value="<?php echo $title; ?>" class="w-full px-3 py-2 border rounded <?php echo (!empty($title_err)) ? 'border-red-500' : ''; ?>" required>
                <span class="text-red-500"><?php echo $title_err; ?></span>
            </div>

            <div class="mb-4">
                <label for="content" class="block mb-2">Content:</label>
                <textarea id="content" name="content" class="w-full px-3 py-2 border rounded <?php echo (!empty($content_err)) ? 'border-red-500' : ''; ?>" required><?php echo $content; ?></textarea>
                <span class="text-red-500"><?php echo $content_err; ?></span>
            </div>

            <div class="mb-4">
                <label for="excerpt" class="block mb-2">Excerpt:</label>
                <textarea id="excerpt" name="excerpt" class="w-full px-3 py-2 border rounded"><?php echo $excerpt; ?></textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="category_id" name="category_id" class="w-full px-3 py-2 border rounded <?php echo (!empty($category_err)) ? 'border-red-500' : ''; ?>" required>
